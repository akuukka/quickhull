...
namespace quickhull {
	...
	namespace mathutils {
		...
		template <typename T>
		inline Vector3<T> getTriangleUnitNormal(const Vector3<T>& a,const Vector3<T>& b,const Vector3<T>& c) {
			// We want to get (a-c).crossProduct(b-c).getNormalized() without constructing temp vectors
			T x = a.x - c.x;
			T y = a.y - c.y;
			T z = a.z - c.z;
			T rhsx = b.x - c.x;
			...
			T py = z * rhsx - x * rhsz ;
			T pz = x * rhsy - y * rhsx ;
			T len = std::sqrt(px*px+py*py+pz*pz);
			return Vector3<T>(px/len,py/len,pz/len);
		}
